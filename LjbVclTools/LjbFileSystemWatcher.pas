...
type
  ...
  TLjbFileSystemWatcher = class(TComponent)
  private
    FActived: Boolean;
    FWatchedDir: string;
...
procedure  Register;
begin
  RegisterComponents('LjbTools', [TLjbFileSystemWatcher]);
end;
...
{ TLjbFileSystemWatcher }

procedure TLjbFileSystemWatcher.Change;
begin
...
constructor TLjbFileSystemWatcher.Create(AOwner: TComponent);
begin
...
destructor TLjbFileSystemWatcher.Destroy;
begin
...
function TLjbFileSystemWatcher.NotifyOptionFlags: DWORD;
begin
  Result := 0;
...
procedure TLjbFileSystemWatcher.SetActived(const Value: Boolean);
begin
...
procedure TLjbFileSystemWatcher.SetFilters(const Value: TNotifyFilters);
begin
...
procedure TLjbFileSystemWatcher.SetOnChange(const Value: TFileDealMethod);
begin
  FOnChange := Value;
...
procedure TLjbFileSystemWatcher.SetWatchedDir(const Value: string);
begin
...
procedure TLjbFileSystemWatcher.SetWatchSubTree(const Value: Boolean);
begin
...
procedure TLjbFileSystemWatcher.Start;
begin
  if csDesigning in ComponentState then
    Exit;
...
procedure TLjbFileSystemWatcher.Stop;
begin
  if csDesigning in ComponentState then
    Exit;
  if Assigned(FThread) then begin
