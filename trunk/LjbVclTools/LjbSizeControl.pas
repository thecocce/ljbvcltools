...
// -----------------------------------------------------------------------------
// Component Name:  TLjbSizeControl
// Module:          SizerControl
...
//    Move, Copy or both TLjbSizeControl actions
//21 Nov 2001
...
type
  ...
  TLjbSizeControlEvent = procedure (Sender: TObject; ControlRect: TRect) of object;
  ...
  TLjbSizeControl = class(TCustomControl)
  private
    fTarget: TControl;
    ...
    fOnMoving: TLjbSizeControlEvent;
    fOnResizing: TLjbSizeControlEvent;
    fOnMoved: TLjbSizeControlEvent;
    fOnResized: TLjbSizeControlEvent;
  ...
  published
    ...
    property OnMoving: TLjbSizeControlEvent read fOnMoving write fOnMoving;
    property OnResizing: TLjbSizeControlEvent read fOnResizing write fOnResizing;
    property OnMoved: TLjbSizeControlEvent read fOnMoved write fOnMoved;
    property OnResized: TLjbSizeControlEvent read fOnResized write fOnResized;
...
procedure Register;
begin
  RegisterComponents('LjbTools', [TLjbSizeControl]);
end;
//------------------------------------------------------------------------------
...
var
  i: integer;
begin
  //nb: As TLjbSizeControl does not assign its Parent property,
...
//------------------------------------------------------------------------------
// TLjbSizeControl methods ...
//------------------------------------------------------------------------------

constructor TLjbSizeControl.Create(aOwner: TComponent);
begin
  inherited;
  ...
  if not (aOwner is TCustomForm) then
    raise Exception.create('TLjbSizeControl.create - Owner must be a TCustomFrom');
  fOwnerForm := TCustomForm(aOwner);
  fMyParent := fOwnerForm;
  //nb: Can't add TLjbSizeControls to Parent's control list as that makes it
  //impossible for TLjbSizeControl to be positioned above an aligned control ...
...
destructor TLjbSizeControl.Destroy;
begin
...
procedure TLjbSizeControl.HookFormWindowProc;
begin
...
procedure TLjbSizeControl.UnHookFormWindowProc;
begin
  fOwnerForm.WindowProc := fOldFormWindowProc;
end;
//------------------------------------------------------------------------------

procedure TLjbSizeControl.CMChanged(var Message: TMessage);
begin
  if assigned(fOnTargetChanged) then fOnTargetChanged(self, fTarget);
end;
//------------------------------------------------------------------------------

procedure TLjbSizeControl.CMNewTarget(var Message: TMessage);
var
...
begin
  ...
  //this enables the LjbSizeControl to respond immediately to mouse movements...
...
procedure TLjbSizeControl.NewFormWindowProc(var Msg : TMessage);
  ...
  begin
    ...
    //and make sure we're not just clicking the LjbSizeControl ...
...
begin
  ...
  case Msg.Msg of
    ...
    //Handling TLjbSizeControl's focus (using ParentWindow) is tricky!!
...
procedure TLjbSizeControl.CreateParams(var Params: TCreateParams);
begin
  inherited;
  if not (csDesigning in ComponentState) then
...
procedure TLjbSizeControl.WMGetDlgCode(var Message: TMessage);
begin
  inherited;
...
procedure TLjbSizeControl.WMEraseBkgnd(var Message: TMessage);
begin
  if (csDesigning in ComponentState) then
    inherited else
    Message.Result := 1;
...
procedure TLjbSizeControl.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  if (csDesigning in ComponentState) then
...
function TLjbSizeControl.DoTargetChanging(NewTarget: TControl): boolean;
begin
  result := true;
...
function TLjbSizeControl.GetTargetAlign: TAlign;
begin
...
procedure TLjbSizeControl.SetTargetAlign(NewAlign: TAlign);
begin
...
procedure TLjbSizeControl.UpdateTargetCoords(MouseOp: TMouseOp);
begin
  invalidate;
...
function TLjbSizeControl.GetTargetLeft: integer;
begin
...
procedure TLjbSizeControl.SetTargetLeft(NewLeft: integer);
begin
...
function TLjbSizeControl.GetTargetTop: integer;
begin
...
procedure TLjbSizeControl.SetTargetTop(NewTop: integer);
begin
...
function TLjbSizeControl.GetTargetWidth: integer;
begin
...
procedure TLjbSizeControl.SetTargetWidth(NewWidth: integer);
begin
...
function TLjbSizeControl.GetTargetHeight: integer;
begin
...
procedure TLjbSizeControl.SetTargetHeight(NewHeight: integer);
begin
...
procedure TLjbSizeControl.SetControlActions(ControlActions: TControlActions);
begin
...
procedure TLjbSizeControl.KeyDown(var Key: Word; Shift: TShiftState);
...
procedure TLjbSizeControl.KeyUp(var key: Word; Shift: TShiftState);
begin
  inherited;
  case key of
    VK_UP, VK_DOWN, VK_LEFT, VK_RIGHT:
...
{$IFDEF VER100}
function TLjbSizeControl.GetEnabled: boolean;
begin
...
procedure TLjbSizeControl.SetEnabled(Enable: boolean);
begin
  {$IFDEF VER100}
...
procedure TLjbSizeControl.SetFocus;
begin
...
procedure TLjbSizeControl.SetGridSize(GridSize: integer);
begin
...
procedure TLjbSizeControl.SetTarget(target: TControl);
begin
  if fInternalChanging then fInternalChanging := false
  else if (fTarget = target) or (target is TLjbSizeControl) then exit
  ...
  //to position TLjbSizeControl above aligned Targets ...
  parent := nil;
  fMyParent := fOwnerForm;
  ParentWindow := fMyParent.Handle;
...
procedure TLjbSizeControl.Notification(AComponent: TComponent;
...
procedure TLjbSizeControl.DrawRect;
var
  dc: hDC;
begin
...
procedure TLjbSizeControl.WMKeyDown(var Message: TMessage);
begin
  //nb: GetParentForm() called in TWinControl.DoKeyDown() always returns nil
  //    because TLjbSizeControl has no assigned Parent property, so...
...
procedure TLjbSizeControl.WMKeyUp(var Message: TMessage);
begin
...
procedure TLjbSizeControl.WMSetCursor(var Message: TWMSetCursor);
var
  pt: TPoint;
  rec: TRect;
  i: integer;
...
procedure TLjbSizeControl.MouseDown(Button: TMouseButton;
...
procedure TLjbSizeControl.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  formOffset: TPoint;
...
procedure TLjbSizeControl.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  pt: TPoint;
  rec: TRect;
begin
...
procedure TLjbSizeControl.Paint;
...
function TLjbSizeControl.GetColor: TColor;
begin
  result := canvas.pen.color;
end;
//------------------------------------------------------------------------------

procedure TLjbSizeControl.SetColor(Color: TColor);
begin
