...
type
  ...
  TLjbCalendar = class(TCustomGrid)
  private
    FDate: TDate;
    FViewDate: TDate;
...
procedure Register;
begin
  RegisterComponents('LjbTools', [TLjbCalendar]);
end;

constructor TLjbCalendar.Create(AOwner: TComponent);
begin
...
procedure TLjbCalendar.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;
procedure TLjbCalendar.MouseToCell(X, Y: Integer; var ACol, ARow: Longint);
var
  Coord: TGridCoord;
begin
...
procedure TLjbCalendar.AcceptDropped(Sender, Source: TObject; X, Y: integer);
var
  ACol, ARow: LongInt;
  Value: string;
begin
...
procedure TLjbCalendar.CellDragOver(Sender, Source: TObject; X, Y: Integer;
      State: TDragState; var Accept: Boolean);
var
  ACol, ARow: LongInt;
begin
...
procedure TLjbCalendar.SetCellString(ACol, ARow, ADay: Integer; Value: string);
var
  i, j: integer;
  TheCellText: string;
begin
...
function TLjbCalendar.IsLeapYear(AYear: Integer): Boolean;
begin
  Result := (AYear mod 4 = 0) and ((AYear mod 100 <> 0) or (AYear mod 400 = 0));
end;

function TLjbCalendar.DaysPerMonth(AYear, AMonth: Integer): Integer;
const
...
function TLjbCalendar.DaysThisMonth: Integer;
begin
  Result := DaysPerMonth(Year, Month);
end;

{procedure TLjbCalendar.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var
  TheText: string;
begin
...
begin
  ...
  with ARect, Canvas do
    begin
      if dz<>'' then
      begin
      Font.Color :=FColor;          //新历字体颜色  旧历有节 
      Font.Size:=Font.Size+AddSize;
      Font.Style:=Font.Style+[fsBold];
      TextRect(ARect, Left +10,Top +2, TheText+sf);   //旧历有节时的 新历显示  
      Font.Size:=Font.Size-AddSize;
      Font.Style:=Font.Style-[fsBold];
      Font.Color :=clRed;         //旧历字体颜色  旧历有节
      TextOut(ARect.Left + Font.Size+10, ARect.Top + (Font.Size+AddSize)*2-3, dz);  //旧历节日名称 显示
      end
      else
      begin
       if sf<>'' then
          begin
            ...
            Font.Size:=Font.Size-AddSize;
            Font.Style:=Font.Style-[fsBold];
            Font.Color :=clMaroon ;          //新历字体颜色  新历有节气 
          ...
          begin
            ...
            Font.Color :=FColor;         // 新历字体颜色  新历没节 星期几标题字体色
            Font.Size:=Font.Size+AddSize;
            TextRect(ARect, Left+TitleLeft,Top+TitleTop, TheText+sf); 
            Font.Size:=Font.Size-AddSize;
            Font.Style:=Font.Style-[fsBold];
          end;
       Font.Color :=clPurple;             //旧历字颜色  旧历没节
       TextOut(ARect.Left + Font.Size+10, ARect.Top + (Font.Size+AddSize)*2-3, ry);
      end;
      end;
  end;

function TLjbCalendar.GetCellText(ACol, ARow: Integer): string;
var
  DayNum: Integer;
begin
...
function TLjbCalendar.SelectCell(ACol, ARow: Longint): Boolean;
begin
...
procedure TLjbCalendar.SetCalendarDate(Value: TDate);
begin
  FDate := Value;
  UpdateCalendar;
  Change;
...
function TLjbCalendar.StoreCalendarDate: Boolean;
begin
  Result := not FUseCurrentDate;
end;

function TLjbCalendar.GetDateElement(Index: Integer): Integer;
var
...
procedure TLjbCalendar.SetDateElement(Index: Integer; Value: Integer);
var
  AYear, AMonth, ADay: Word;
begin
  if Value > 0 then
...
procedure TLjbCalendar.SetStartOfWeek(Value: TDayOfWeek);
begin
...
procedure TLjbCalendar.SetUseCurrentDate(Value: Boolean);
begin
...
procedure TLjbCalendar.ChangeMonth(Delta: Integer);
var
  AYear, AMonth, ADay: Word;
  NewDate: TDate;
  CurDay: Integer;
...
procedure TLjbCalendar.PrevMonth;
begin
  ChangeMonth(-1);
end;

procedure TLjbCalendar.NextMonth;
begin
  ChangeMonth(1);
end;

procedure TLjbCalendar.NextYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year + 1;
end;

procedure TLjbCalendar.PrevYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year - 1;
end;

procedure TLjbCalendar.UpdateCalendar;
var
  AYear, AMonth, ADay: Word;
  FirstDate: TDate;
begin
...
procedure TLjbCalendar.WMSize(var Message: TWMSize);
var
  GridLines: Integer;
begin
...
function TLjbCalendar.DaysOfLunarYear(y: integer): integer;
var
  i, sum: integer;
begin
  sum:= 348;       //29 * 12
...
// 返回农历 y年闰月的天数
function TLjbCalendar.DaysOfLeapMonth(y: integer): integer;
begin
...
//返回农历 y年闰哪个月 1-12 , 没闰返回 0
function TLjbCalendar.leapMonth(y: integer): integer;
var
  lm: Word;
begin
...
//返回农历 y年m月的天数
function TLjbCalendar.DaysOfMonth(y, m: integer): integer;
var
...
//算出农历, 传入公历日期, 返回农历日期
function TLjbCalendar.ToLunar(TheDate: TDate): THzDate;
var
...
function TLjbCalendar.GetGZ(y, m: integer; TheDate: TDate): TGzDate;
var
  term: TDate;
  sy, sm, sd: Word;
begin
...
// 算出公历, 传入农历日期控件, 返回公历
function TLjbCalendar.ToGreg(objDate: THzDate): TDate;
var
...
// 检查农历日期是否合法
function TLjbCalendar.ChkHzDate(objDate: THzDate): Boolean;
begin
...
// 某年的第n个节气为几日(从0小寒起算)
function TLjbCalendar.sTerm(y, n: integer): TDateTime;
var
  temp: TDateTime;
  t: real;
  i: Int64;
...
// 传入 offset 返回干支, 0=甲子
function TLjbCalendar.cyclical(num: integer): string;
begin
   Result:= Gan[num mod 10] + Zhi[num mod 12]+'('+Animals[num mod 12]+'年)';
end;

function TLjbCalendar.FormatLunarDay(day:integer): string;
begin
  case day of
...
function TLjbCalendar.FormatLunarMonth(month:integer;isLeap:boolean): string;
begin
  case month of
...
function TLjbCalendar.FormatLunarYear(year:integer): string;
var
  temp: integer;
  zero: string;
begin
...
// 取得指定日期的节气
function TLjbCalendar.GetJQ(TheDate: TDate): string;
var
  jq: Integer;
  term: TDateTime;
begin
...
function TLjbCalendar.GetsFtv(TheDate: TDate): string;
var
 sf:string;
 jlsl:integer;
 begin
...
function TLjbCalendar.GetlFtv(TheDate: ThzDate): string;
var
 sf,m,d:string;
 jlsl:integer;
 begin
