...
const
  ...
  RomBiosDate = Pointer($000FFFF5);

  RomBiosName = Pointer($000FE061);

  RomBiosCopyright = Pointer($000FE091);

  RomBiosId = Pointer($000FEC71);
...
type
  ...
  TReadRomBiosMethod = (
    ...
    rrbmAutomatic, {自动检测系统类型以使用正确的方法}

    rrbmGeneric,   {使用16位COM程序去堆存BIOS       }

    rrbmMemory,    {从内存读取(Win9x)               }

    rrbmPhysical   {从物理内存读取(WinNT)           }
